...
scripts_folder = airflow_home + '/scripts'
data_sample_folder = scripts_folder + '/' + folder_name
if  folder_name in os.listdir(scripts_folder):
    shutil.rmtree(data_sample_folder)
os.mkdir(data_sample_folder)
...
for i in tqdm(range(len(list_date))):
    ...
    path = os.path.join(data_sample_folder,str(create_at)) + '.parquet'
